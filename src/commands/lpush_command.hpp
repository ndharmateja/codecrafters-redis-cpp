#ifndef LPUSH_COMMAND_HPP
#define LPUSH_COMMAND_HPP
...
public:
    explicit LpushCommand(const std::vector<std::string> &command_parts)
    {
        // Parse key and values
        if (command_parts.size() < 3)
            throw InvalidCommandStructureError("LPUSH should have atleast 3 parts.");
        key = command_parts[1];
